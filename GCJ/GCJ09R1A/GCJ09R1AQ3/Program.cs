...
namespace GCJ09R1AQ3
{
    class Program
    {
        ...
        private static double OtherSolve(double[] cache, int c, int p, int n)
        {
            ...
            {
                // p!/(p-i)!/i!*(c-p)!/(c-p-n+i)!/(n-i)!/c!*(c-n)!*n!
                // p!*(c-p)!*(c-n)!*n!/(p-i)!/i!/(c-p-n+i)!/(n-i)!/c!
                double probScenario = Hmm(p, i, c, n);// ((double)Choose(p, i) * (double)Choose(c - p, n - i)) / (double)Choose(c, n);
        ...
        public static double Hmm(int p, int i, int c, int n)
        {
            //double otherRes = ((double)Choose(p, i) * (double)Choose(c - p, n - i)) / (double)Choose(c, n);
            return ((double)Choose2(p, i) * (double)Choose2(c - p, n - i)) / (double)Choose2(c, n);
        }
        ...
        public static double Choose2(long n, long k)
        {
            double res = 1;
            long currentK = k;
            for (long i = n; i >= n - k + 1; i--)
